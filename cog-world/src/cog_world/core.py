"""M26 世界环境模块主实现。
M26 world environment — main implementation.

职责：2-D 竞技场上的食物/危险/资源生成与交互。
Responsibilities: generate and mediate food/hazards/resources on a 2-D
arena.
- 智能体（M25 Body）在世界中注册后占据一个位置。
  An agent (M25 Body) occupies a position once registered in the world.
- observe(id)：局部观测向量（位置归一化 + 食物气味 + 危险接近度 + 自身状态）。
  observe(id): local observation vector (normalized position + food
  scent + danger proximity + self state).
- step(id, action)：身体执行动作 + 环境交互（吃到食物 -> eat 效果、
  踩到危险 -> damage），返回 (obs, reward)。
  step(id, action): the body acts + environment interactions (eat food
  -> eat effect; step on a hazard -> damage); returns (obs, reward).
- reward（内在奖励雏形）：能量增量 + 完整性增量 - 危险惩罚 - 存活开销。
  reward (a proto intrinsic reward): energy delta + integrity delta -
  hazard penalty - living cost.
"""
...
#: Observation dimension (position 2 + food scent 1 + danger proximity 1
#: + energy 1 + integrity 1)
OBS_DIM = 6
#: 吃到食物的判定半径 / radius for eating food
EAT_RADIUS = 0.5
#: 踩到危险的判定半径与伤害 / hazard radius and damage
DANGER_RADIUS = 0.5
DANGER_DAMAGE = 0.1
#: 每步存活开销（鼓励高效行为）/ per-step living cost (encourages efficiency)
LIVING_COST = 0.01


class World:
    """2-D 网格世界（连续坐标）。
    2-D arena world (continuous coordinates).

    Args:
        size: 竞技场边长。Arena side length.
        n_food: 食物数量。Number of food items.
        n_danger: 危险区域数量。Number of hazard zones.
        seed: 随机种子（决定食物/危险布局，可复现）。
            Random seed (determines the food/hazard layout; reproducible).
    """
    ...
    def reset(self, seed: int | None = None) -> None:
        """重新生成食物与危险布局（智能体保留）。
        Regenerate the food/hazard layout (agents are kept)."""
    ...
    def add_agent(self, body: Body | None = None, position=None) -> int:
        """注册一个智能体身体，返回其 ID。
        Register an agent body; returns its ID."""
        if body is None:
    ...
    def observe(self, agent_id: int) -> np.ndarray:
        ...
        Local observation (OBS_DIM,): normalized position, food scent,
        danger proximity, self state."""
    ...
    def step(self, agent_id: int, action: Action) -> StepResult:
        """智能体执行一步：身体动作 + 环境交互。
        One agent step: body action + environment interaction.
        ...
        # 1. The body acts (no "eat" — how much is eaten is decided by
        # the environment).
        ...
        # 4. 内在奖励雏形：趋利避害 + 存活开销
        # 4. Proto intrinsic reward: seek benefit, avoid harm, minus the
        # living cost.
        reward = (
    ...
    def _try_eat(self, pos: Tuple[float, float]) -> float:
        ...
        Eat the first food within the radius; returns the energy gain
        (0 if nothing was eaten)."""
        for i, (fx, fy) in enumerate(self.food):
            if abs(fx - pos[0]) <= EAT_RADIUS and abs(fy - pos[1]) <= EAT_RADIUS:
                self.food.pop(i)
                return 0.3  # 一份食物的能量 / energy per food item
        return 0.0

    def _food_scent(self, pos: Tuple[float, float]) -> float:
        """食物气味 ∈ [0,1]：最近食物距离的负指数。
        Food scent ∈ [0,1]: negative exponential of the distance to the
        nearest food."""
        if not self.food:
            return 0.0
    ...
    def _danger_proximity(self, pos: Tuple[float, float]) -> float:
        ...
        Danger proximity ∈ [0,1]: 1 inside the hazard radius, decaying
