...
Homeostatic rewards (core idea ported from M12 cog-drives).

模型的唯一外部回报来源：缺口缩减。
The model's ONLY external reward source: deficit reduction.
没有任务奖励、没有人工偏好、没有"正确答案"。
No task rewards, no human preferences, no "correct answers".

模型（设定点思想来自 DriveSystem）：
Formulation (setpoint idea from DriveSystem):
    每个稳态变量 v ∈ [0, 1]，设定点 sp；
    Each homeostatic variable v ∈ [0, 1] with setpoint sp;
    缺口 deficit = clamp(sp - v, 0, 1)；
    deficit = clamp(sp - v, 0, 1);
    奖励 = D(before) - D(after)，即一步内总缺口的缩减量。
    reward = D(before) - D(after): total deficit reduction in one step.
"""
...
# (能量设定点, 完整性设定点) / (energy setpoint, integrity setpoint)
...
def deficits(intero: Sequence[float],
    ...
    """各稳态变量的缺口向量，形状 (2,)。
    Per-variable deficit vector, shape (2,)."""
...
def homeostatic_reward(before: Sequence[float],
    ...
    One-step homeostatic reward: deficit reduction. Positive when eating
    under a deficit, negative when a variable deteriorates."""
...
def in_crisis(intero: Sequence[float],
    ...
    True if any variable's deficit exceeds the crisis threshold
    (mirrors M12's in_crisis)."""
